...
class Fridge(Container, Device):
    def __init__(self):
        ...
        self.properties[
            "isMoveable"
        ] = False  # A fridge is too heavy to move (and doesn't really need to be moved for this simulation)

        self.properties["isOn"] = True  # A frridge is always on
        self.properties[
            "isActivatable"
        ] = False  # A fridge essentially never is turned off (unless it's unplugged, which is irelevant for this simulation)

        # Set critical properties
        self.properties[
            "minTemperature"
        ] = 4.0  # Minimum temperature of the fridge (in degrees Celsius)
        self.properties[
            "tempDecreasePerTick"
        ] = 5.0  # How much the temperature decreases per tick (in degrees Celsius)
            ...
            if self.properties["isOpen"] == False:
                ...
                for obj in objectsOnStove:
                    ...
                    newTemperature = (
                        ...
                        - self.properties["tempDecreasePerTick"]
                    )
        ...
        if self.properties["isOpen"]:
            ...
            if len(effectiveContents) > 0:
                ...
                for i in range(len(effectiveContents)):
                    if (i == len(effectiveContents) - 1) and (
                        len(effectiveContents) > 1
                    ):
                        outStr += "and "
...
class FoodItem(GameObject):
    def __init__(self, foodPrefix, foodName, spoilTime=20):
        ...
        self.properties[
            "foodPrefix"
        ] = foodPrefix  # The prefix to use when referring to the food item (e.g. "a", "some", "the", etc.)

        # Set critical properties
        self.properties["isFood"] = True
        self.properties[
            "spoilTime"
        ] = spoilTime  # How long the food item lasts before it spoils (in steps)
        self.properties[
            "maxTemperature"
        ] = 4.0  # Maximum temperature of the food item (in degrees Celsius).  If the food item is hotter than this, it will eventually spoil.
        self.properties[
            "timeAboveMaxTemp"
        ] = 0  # How long the food item has been above the maximum temperature (in steps)
        self.properties["isSpoiled"] = False  # Whether the food item is spoiled or not
        self.properties[
            "isAboveMaxTemp"
        ] = True  # Whether the food item is above the maximum temperature or not (helpful for scoring)
...
class Counter(Container):
    ...
    def __init__(self):
        ...
        self.properties[
            "isOpenable"
        ] = False  # A counter pit is a flat surface that can't be closed (i.e always open)
