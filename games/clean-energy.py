...
class PowerPlant(GameObject):
    def __init__(self, name, efficiency, capasity):
        ...
        self.properties[
            "capacityKW"
        ] = capasity  # capacity of the power plant in KW. The actual output power is effiency times capacity
...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        self.CO2_threshold = (
            16  # If there are 5 regions, the agent can take one wrong action
        )
        ...
        fire_power_resource = self.random.choices(
            ["sun", "wind", "water"], k=num_fire_power_stations
        )
        fire_power = list(zip(fire_power, fire_power_resource))

        power_stations = fire_power + self.random.choices(
            clean_power, k=num_regions - num_fire_power_stations
        )
        # shuffle
        ...
        for n, power_station in enumerate(power_stations):
            ...
            if (
                power_station[0] == "solar farm"
                and power_station[1] != "sun"
                or power_station[0] == "wind farm"
                and power_station[1] != "wind"
                or power_station[0] == "hydroelectric power station"
                and power_station[1] != "water"
            ):
                efficiency = 0.1
            else:
                efficiency = 1
            power_station = PowerPlant(
                power_station[0], efficiency, self.capacity_per_plant
            )
    ...
    def generatePossibleActions(self):
        ...
        for objReferent1, objs1 in allObjects.items():
            for power_station in [
                ...
                for obj1 in objs1:
                    self.addAction(
                        "change " + objReferent1 + " to " + power_station,
                        ["change", obj1, power_station],
                    )
    ...
    def actionChange(self, region, power_station):
        ...
        # check if the region is valid
            ...
            if (
                power_station == "solar farm"
                and region.properties["resource"] != "sun"
                or power_station == "wind farm"
                and region.properties["resource"] != "wind"
                or power_station == "hydroelectric power station"
                and region.properties["resource"] != "water"
            ):
                efficiency = 0.1
            else:
                efficiency = 1
            new_power_station = PowerPlant(
                power_station, efficiency, self.capacity_per_plant
            )
    ...
    def calculateScore(self):
        ...
        for obj in allObjects:
            ...
            if "fossil-fuel power station" in power_plant.name:
                ...
                break
            capasity = (
                power_plant.properties["running_efficiency"]
                * power_plant.properties["capacityKW"]
            )
