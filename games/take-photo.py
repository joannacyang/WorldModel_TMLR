...
class GameObject:
    def __init__(self, name):
        ...
        self.properties[
            "temperature"
        ] = 20.0  # Initialize everything to have a starting temperature of 20 degrees C
    ...
    def placeObjectInContainer(self, obj):
        ...
        if not (self.getProperty("isContainer") == True):
            ...
            return (
                "The "
                + self.name
                + " is not a container, so things can't be placed there.",
                False,
            )
        ...
        if not (self.getProperty("isOpen") == True):
            # If not, then it can't be placed in a container
            return (
                "The " + self.name + " is closed, so things can't be placed there.",
                False,
            )
        ...
        self.addObject(obj)
        return (
            "The " + obj.getReferents()[0] + " is placed in the " + self.name + ".",
            True,
        )
    ...
    def takeObjectFromContainer(self, obj):
        ...
        if not (self.getProperty("isContainer") == True):
            ...
            return (
                "The "
                + self.name
                + " is not a container, so things can't be removed from it.",
                None,
                False,
            )
        ...
        if not (self.getProperty("isOpen") == True):
            # If not, then it can't be removed from a container
            return (
                "The " + self.name + " is closed, so things can't be removed from it.",
                None,
                False,
            )
        ...
        if obj not in self.contains:
            return (
                "The " + obj.name + " is not contained in the " + self.name + ".",
                None,
                False,
            )
        ...
        obj.removeSelfFromContainer()
        return (
            "The " + obj.getReferents()[0] + " is removed from the " + self.name + ".",
            obj,
            True,
        )
...
class Camera(Device):
    def __init__(self, random_seed):
        ...
        # Set critical properties
        self.properties["shutter_speed_dial"] = [
            "1/4000",
            "1/2000",
            "1/1000",
            "1/500",
        ...
        self.properties["current_shutter_speed"] = self.random.randint(
            0, len(self.properties["shutter_speed_dial"]) - 1
        )
        self.properties["current_iso"] = self.random.randint(
            0, len(self.properties["iso_dial"]) - 1
        )
        self.properties["current_aperture"] = self.random.randint(
            0, len(self.properties["aperture_dial"]) - 1
        )
    ...
    def rotate_dial(self, which_dial="aperture", clockwise=True):
        ...
        if which_dial == "aperture":
            self.properties["current_aperture"] = (
                self.properties["current_aperture"]
                + len(self.properties["aperture_dial"])
                + diff
            ) % len(self.properties["aperture_dial"])
            ...
            self.properties["current_iso"] = (
                self.properties["current_iso"] + len(self.properties["iso_dial"]) + diff
            ) % len(self.properties["iso_dial"])
            outStr = "You rotated the iso dial to %s" % (
                self.properties["iso_dial"][self.properties["current_iso"]]
            )
        elif which_dial == "shutter speed":
            self.properties["current_shutter_speed"] = (
                self.properties["current_shutter_speed"]
                + len(self.properties["shutter_speed_dial"])
                + diff
            ) % len(self.properties["shutter_speed_dial"])
    ...
    def shutter(self):
        current_focus_ = (
            "nothing" if self.current_focus is None else self.current_focus.name
        )
        self.properties["photo"] = [
            current_focus_,
            copy.copy(self.properties["current_shutter_speed"]),
            copy.copy(self.properties["current_aperture"]),
            copy.copy(self.properties["current_iso"]),
        ]
        outStr = (
            "You took a photo of %s, with shutter speed of %s, aperture of %s, and iso of %s."
            % (
                self.properties["photo"][0],
                self.properties["shutter_speed_dial"][self.properties["photo"][1]],
                self.properties["aperture_dial"][self.properties["photo"][2]],
                self.properties["iso_dial"][self.properties["photo"][3]],
            )
        )
        return outStr
    ...
    def makeDescriptionStr(self, makeDetailed=False):
        ...
        outStr = []
        outStr.append(
            "A loaded %s, the current shutter speed, aperture, and iso are %s, %s, and %s, it is currently focusing on %s."
            % (
                self.name,
                self.properties["shutter_speed_dial"][
                    self.properties["current_shutter_speed"]
        ...
        outStr.append(
            "The min/max available shutter speed is %s and %s."
            % (
                self.properties["shutter_speed_dial"][0],
                self.properties["shutter_speed_dial"][-1],
            )
        )
        outStr.append(
            "The min/max available aperture is %s and %s."
            % (
                self.properties["aperture_dial"][0],
                self.properties["aperture_dial"][-1],
            )
        )
        outStr.append(
            "The min/max available iso is %s and %s."
...
class TextGame:
    ...
    def sample_task(self, exist_foods):
        ...
        target_aperture_idx = self.random.choice(
            range(len(self.camera.properties["aperture_dial"]))
        )
        self.target_aperture = self.camera.properties["aperture_dial"][
            target_aperture_idx
        ]
        target_speed_idx = self.random.choice(
            range(len(self.camera.properties["shutter_speed_dial"]))
        )
        self.target_shutter_speed = self.camera.properties["shutter_speed_dial"][
            target_speed_idx
        ]
        target_iso_idx = self.random.choice(
            range(len(self.camera.properties["iso_dial"]))
        )
        self.target_iso = self.camera.properties["iso_dial"][target_iso_idx]

        self.useful_info += [target_aperture_idx, target_speed_idx, target_iso_idx]

    # Get the task description for this game
    def getTaskDescription(self):
        return (
            "Your task is to take a nice picture of %s, using a camera with shutter speed of %s, aperture of %s, and iso of %s."
            % (
                self.target_food.name,
                self.target_shutter_speed,
        ...
        for objReferent, objs in allObjects.items():
            for obj in objs:
                ...
                self.addAction(
                    ...
                    + objReferent
                    + " from "
                    + obj.parentContainer.getReferents()[0],
                    ["take", obj],
                )
            ...
            for objReferent2, objs2 in allObjects.items():
                for obj1 in objs1:
                    for obj2 in objs2:
                        if obj1 != obj2:
                            self.addAction(
                                "use " + objReferent1 + " on " + objReferent2,
                                ["use", obj1, obj2],
                            )
    ...
    def actionTake(self, obj):
        ...
        if obj.parentContainer is None:
    ...
    def calculateScore(self):
        ...
        self.score = 0
        if (
            self.camera.properties["photo"] is not None
            and self.camera.properties["photo"][0] == self.target_food.name
            and self.camera.properties["shutter_speed_dial"][
                self.camera.properties["photo"][1]
            ]
            == self.target_shutter_speed
            and self.camera.properties["aperture_dial"][
                self.camera.properties["photo"][2]
            ]
            == self.target_aperture
            and self.camera.properties["iso_dial"][self.camera.properties["photo"][3]]
            == self.target_iso
        ):
            self.score = 1
            self.gameOver = True
