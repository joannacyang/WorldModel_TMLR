...
class ElectricalObject(GameObject):
    ...
    def disconnect(self, terminal):
        if self.connects[terminal][0] is not None:
            ...
            obj_connects_to.properties["connects"][connected_terminal] = (None, None)
        self.connects[terminal] = (None, None)
        self.properties["connects"][terminal] = (None, None)
...
class Battery(ElectricalObject):
    def __init__(self, name):
        ...
        # Set critical properties
        self.properties["connects"] = {"cathode": (None, None), "anode": (None, None)}
...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        self.useful_info.append(self.conductive)
    ...
    def calculateScore(self):
        ...
        if ("red box" in self.test_obj.parentContainer.name and self.conductive) or (
            "black box" in self.test_obj.parentContainer.name and not self.conductive
        ):
            self.score += 1
            self.gameOver = True
            self.gameWon = True
        elif (
            "red box" in self.test_obj.parentContainer.name and not self.conductive
