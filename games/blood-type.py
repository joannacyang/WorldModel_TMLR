...
class Patient(GameObject):
    def __init__(self, blood_type, Rh):
        ...
        self.properties["given_blood"] = None
...
class Blood(GameObject):
    ...
    def getReferents(self):
        return [
            f"Type {self.properties['blood_type'][0]} {self.properties['blood_type'][1]} {self.name}"
        ]
...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        # Add a patient and randomly choose a blood type for the patient
        ...
        self.useful_info.append((patient_blood_type, patient_rh))
        ...
        for blood_type in blood_types:
            for rh in Rh:
                if (
                    blood_type in self.blood_type_lut[patient_blood_type]
        ...
        available_accepted_types = self.random.sample(
            matched_types, k=num_avail_accepted_types
        )
        available_unaccepted_types = self.random.sample(
            unmatched_types, k=num_avail_unaccepted_types
        )
        ...
        for blood_type, rh in available_accepted_types:
            ...
            self.useful_info.append((blood_type, rh))
            ...
            for objReferent2, objs2 in allObjects.items():
                for obj1 in objs1:
                    for obj2 in objs2:
                        if obj1 != obj2:
                            self.addAction(
                                "give " + objReferent1 + " to " + objReferent2,
                                ["give", obj1, obj2],
                            )
    ...
    def actionGive(self, blood, patient):
        ...
        elif type(blood.parentContainer) != Agent:
            return f"You need to take the blood first."
        else:
            patient.properties["given_blood"] = blood.properties["blood_type"]
    ...
    def calculateScore(self):
        ...
        allObjects = self.rootObject.getAllContainedObjectsRecursive()
        for obj in allObjects:
            if type(obj) == Patient and obj.properties["given_blood"] is not None:
                given_blood_type, given_blood_Rh = obj.properties["given_blood"]
                if (
                    given_blood_type
                    in self.blood_type_lut[obj.getProperty("blood_type")[0]]
                    and given_blood_Rh
                    in self.blood_type_rh_lut[obj.getProperty("blood_type")[1]]
                ):
                    self.score += 1
                    self.gameOver = True
                    self.gameWon = True
