...
class Room(Container):
    def __init__(self, name, outdoor=False):
        ...
        self.properties["connects"] = []  # other rooms that this room connects to
    ...
    def makeDescriptionStr(self, makeDetailed=False):
        ...
        outStr += "You also see:\n"
        for connected_room_name in self.properties["connects"]:
            outStr += f"\t a way to the {connected_room_name}\n"

        return outStr

    def connectsTo(self, room):
        connected = False
        for r in self.properties["connects"]:
            if r == room.name:
                connected = True
                break
        return connected
...
class World(Container):
    ...
    def makeDescriptionStr(self, room, makeDetailed=False):
        ...
        # describe room connection information
        outStr += "You also see:\n"
        for connected_room_name in room.properties["connects"]:
            outStr += f"\t a way to the {connected_room_name}\n"
...
class Agent(Container):
    def __init__(self, use_sunscreen=False, sunburn=False):
        GameObject.__init__(self, "agent")
        Container.__init__(self, "agent")
        self.properties["use_sunscreen"] = use_sunscreen
        self.properties["sunburn"] = sunburn
    ...
    def tick(self):
        if self.parentContainer.getProperty("is_outdoor") and not self.getProperty(
            "use_sunscreen"
        ):
...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        # Connects two rooms
        house.properties["connects"].append(beach.name)
        beach.properties["connects"].append(house.name)
        ...
        detergent = Items("detergent")
        water = Items("water")
        house.addObject(detergent)
        house.addObject(water)

        # Add objects to the beach
        # Add a ball
        ball = GameObject("ball")
        chair = GameObject("chair")
        beach.addObject(ball)
        beach.addObject(chair)
        self.target = ball
    ...
    def getTaskDescription(self):
        return "It is a summer noon. The sky is clear. Your task is to take a ball from the beach and put it in the box in the house. Protect yourself from sunburn!"
    ...
    def step_action(self, actionStr):
        ...
        if actionVerb == "look around":
            ...
            self.observationStr = self.agent.parentContainer.makeDescriptionStr()
    ...
    def calculateScore(self):
        ...
        if self.agent.properties["sunburn"]:
            ...
            if self.target.parentContainer == self.answer_box:
                self.score = 1
                self.gameOver = True
                self.gameWon = True
