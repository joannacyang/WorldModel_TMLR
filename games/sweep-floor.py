...
class GameObject:
    def __init__(
        ...
        self.properties[
            "isContainer"
        ] = isContainer  # By default, objects are not containers
        ...
        self.properties[
            "isActivatable"
        ] = isActivatable  # By default, objects are not acitvatable
...
class Container(GameObject):
    def __init__(self, name, isOpenable=False, isOpen=True, containerPrefix="in"):
        ...
        self.properties[
            "isOpenable"
        ] = isOpenable  # Can the container be opened (e.g. a drawer, a door, a box, etc.), or is it always 'open' (e.g. a table, a shelf, etc.)
        self.properties[
            "isOpen"
        ] = isOpen  # Is the container open or closed (if it is openable)
        self.properties[
            "containerPrefix"
        ] = containerPrefix  # The prefix to use when referring to the container (e.g. "in the drawer", "on the table", etc.)
            ...
            for obj in self.contains:
                outStr += (
                    "\n".join(
                        [
                            "\t\t" + desc
                            for desc in obj.makeDescriptionStr().strip().split("\n")
                        ]
                    )
                    + "\n"
                )
...
class Garbage(GameObject):
    def __init__(self, name):
        ...
        self.properties[
            "isMoveable"
        ] = False  # We don't allow the garbage to be directly taken by the agent

    def makeDescriptionStr(self, makeDetailed=False):
        if type(self.parentContainer) == World:
...
class GarbageCan(Container):
    def __init__(self, name):
        ...
        self.properties[
            "isMoveable"
        ] = False  # We don't allow the garbage can to be directly moved by the agent
...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        # Add a broom
        broom = Tool("broom")
        world.addObject(broom)

        # Add a dustpan
        dustpan = Container("dustpan")
        world.addObject(dustpan)

        # Add a garbage can
        garbage_can = GarbageCan("garbage can")
        world.addObject(garbage_can)
        ...
        all_garbage = possible_garbage[:num_garbage]

        for garbage_name in all_garbage:
            garbage = Garbage(garbage_name)
            world.addObject(garbage)

            self.useful_info.append(garbage)
            ...
            for objReferent2, objs2 in allObjects.items():
                for obj1 in objs1:
                    for obj2 in objs2:
                        if obj1 != obj2:
                            self.addAction(
                                "dump " + objReferent1 + " to " + objReferent2,
                                ["dump", obj1, obj2],
                            )
                        ...
                        for obj2 in objs2:
                            for obj3 in objs3:
                                if obj1 != obj2 and obj2 != obj3 and obj3 != obj1:
                                    self.addAction(
                                        f"sweep {objReferent1} to {objReferent2} with {objReferent3}",
    ...
    def actionSweep(self, garbage, dustpan, broom):
        ...
        if "dustpan" not in dustpan.name:
            return f"You can't sweep {garbage.name} to {dustpan.name}."
        if "broom" not in broom.name:
    ...
    def actionDump(self, dustpan, garbage_can):
        ...
        if not garbage_can.getProperty("isOpen"):
            return f"The {garbage_can.name} is closed."
    ...
    def step_action(self, actionStr):
        ...
        if actionVerb == "look around":
            ...
            dustpan = action[2]
            broom = action[3]
            self.observationStr = self.actionSweep(garbage, dustpan, broom)

        elif actionVerb == "dump":
            garbage = action[1]
            garbage_can = action[2]
            self.observationStr = self.actionDump(garbage, garbage_can)
