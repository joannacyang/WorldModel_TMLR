...
class Tap(GameObject):
    def __init__(self, name, water_temperature, is_on=False):
        ...
        self.properties[
            "water_temperature"
        ] = water_temperature  # The temperature of the water coming out from the tap
    ...
    def tick(self):
        if (
            self.properties["isOn"]
            and self.parentContainer is not None
            and type(self.parentContainer) == BathTub
        ):
            water_list = []
...
class Thermometer(GameObject):
    ...
    def useWithObject(self, obj):
        ...
        if type(obj) == Agent or type(obj) == Thermometer:
            ...
            return (
                f"The thermometer reads {obj.getProperty('temperature')} Celsius degree.",
                True,
            )
...
class Water(GameObject):
    def __init__(self, temperature):
        ...
        self.properties[
            "isMoveable"
        ] = False  # Water is liquid so we don't allow it to be moved
...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        temperature = self.random.randint(20, 60)
        water = Water(temperature)
        bath_tub.addObject(water)

        self.useful_info.append(temperature)
