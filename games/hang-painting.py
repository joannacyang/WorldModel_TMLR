...
class TextGame:
            ...
            for objReferent2, objs2 in allObjects.items():
                for obj1 in objs1:
                    for obj2 in objs2:
                        if obj1 != obj2:
                            self.addAction(
                                "hang " + objReferent1 + " on " + objReferent2,
                                ["hang", obj1, obj2],
                            )
                        ...
                        for obj2 in objs2:
                            for obj3 in objs3:
                                if obj1 != obj2 and obj2 != obj3 and obj3 != obj1:
                                    self.addAction(
                                        f"hammer {objReferent1} on {objReferent2} with {objReferent3}",
                                        ["hammer", obj1, obj2, obj3],
                                    )
    ...
    def calculateScore(self):
        ...
        for obj in allObjects:
            ...
            if type(obj) == Nail:
                if (
                    obj.parentContainer != self.target_wall
                    and type(obj.parentContainer) == Wall
                ):
                    self.gameOver = True
                    self.gameWon = False
                    self.score = 0
                    ...
                    obj.parentContainer == self.target_wall
                    and obj.containsItemWithName(self.target_picture.name)
                ):
                    self.gameOver = True
                    self.gameWon = True
                    self.score = 1
