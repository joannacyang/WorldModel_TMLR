...
class HeatSource(Container):
    def __init__(
        ...
        self.properties[
            "maxTemperature"
        ] = maxTemperature  # Maximum temperature of the heat source (in degrees Celsius)
        self.properties[
            "tempIncreasePerTick"
        ] = tempIncreasePerTick  # How much the temperature increases per tick (in degrees Celsius)
            ...
            for obj in containedObjects:
                if obj.properties["temperature"] > self.properties["maxTemperature"]:
                    newTemperature = max(
                        ...
                        - self.properties["tempIncreasePerTick"],
                        self.properties["maxTemperature"],
                    )
                else:
                    newTemperature = min(
                        obj.properties["temperature"]
    ...
    def makeDescriptionStr(self, makeDetailed=False):
        ...
        if len(self.contains) == 0:
            ...
            outStr += (
                ...
                + self.properties["containerPrefix"]
                + " it: "
            )
...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        # Add a copper ingot
        copper = Substance(
            "copper", "copper (liquid)", "copper (steam)", "ingot", 2562, 1085, 20
        )
            ...
            for objReferent2, objs2 in allObjects.items():
                for obj1 in objs1:
                    for obj2 in objs2:
                        if obj1 != obj2:
                            self.addAction(
                                "open " + objReferent1 + " with " + objReferent2,
                                ["open", obj1, obj2],
                            )
    ...
    def actionOpenWith(self, door, key):
        ...
        if (
            type(key) != Substance
            or key.getProperty("solidShapeName") != "key"
            or key.getProperty("stateOfMatter") != "solid"
        ):
