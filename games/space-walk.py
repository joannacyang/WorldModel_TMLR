...
class Room(Container):
    ...
    def connectsToOuterSpace(self, visited):
        ...
        for r in self.connects:
            if r in visited:
                continue
            elif self.connects[r] is not None and not self.connects[r].getProperty(
                "is_open"
            ):
                continue
    ...
    def makeDescriptionStr(self, makeDetailed=False):
        ...
        if curr_room is not None:
            if self.properties["is_open"]:
                outStr = (
                    f"a {self.name} to the {self.connects[curr_room].name} that is open"
                )
            else:
...
class Agent(Container):
    ...
    def tick(self):
        if self.parentContainer.connectsToOuterSpace([]) and not self.getProperty(
            "wearSpaceSuit"
        ):
