...
class Tool(GameObject):
    def __init__(self, name, type):
        GameObject.__init__(self, name)
        # Set critical properties
        self.properties["type"] = type  # tool type
...
class Soil(GameObject):
    ...
    def makeDescriptionStr(self, makeDetailed=False):
        if self.properties["wet"]:
            return f"wet {self.name}"
        else:
            return f"dry {self.name}"
...
class World(Container):
    ...
    def makeDescriptionStr(self, makeDetailed=False):
        outStr = "You are in the garden.  In the garden, you see: \n"
        for obj in self.contains:
            outStr += "\t" + obj.makeDescriptionStr() + "\n"
...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        shovel = Tool("shovel", "shovel")
        world.addObject(shovel)
        hammer = Tool("hammer", "hammer")
        world.addObject(hammer)
        screwer = Tool("screwer", "screwer")
        world.addObject(screwer)
        # Add a sink
        sink = Sink()
        world.addObject(sink)
        # Add some water containers
        bucket = WaterContainer("bucket")
        world.addObject(bucket)
        jug = WaterContainer("jug")
        world.addObject(jug)
        # Add a tree
        tree = Tree("tree")
    ...
    def getTaskDescription(self):
        return f"Your task is to plant the tree and water it."
    ...
    def actionPut(self, objToMove, newContainer):
        ...
        # A tree cannot be put into a hole with soil in it
        if type(newContainer) == Hole and type(objToMove) == Tree:
            has_soil = False
            for obj in newContainer.contains:
                if type(obj) == Soil:
                    has_soil = True
            if has_soil:
                return "There is no room to plant a tree in that hole."
    ...
    def actionDig(self, tool):
        if type(tool.parentContainer) != Agent:
            return f"To use {tool.name}, you need to put it in your inventory first."
        else:
            if tool.getProperty("type") != "shovel":
                return f"You can't dig with {tool.name}."
            else:
                # if dig with a shovel, generate a hole and put some soil to the agent's inventory
                self.rootObject.addObject(Hole("hole"))
    ...
    def actionPour(self, water, target):
        if type(water) != Water:
            ...
            # if the target is able to hold water, add it to the target
            if target.getProperty("isWaterContainer"):
                target.addObject(water)
            # if the target is soil, make the soil wet
            elif type(target) == Soil:
                target.properties["wet"] = True
                del water
            # otherwise remove water from the environment
            else:
                del water
    ...
    def step_action(self, actionStr):
        ...
        if actionVerb == "look around":
            ...
            # dig a hole
            toolToDig = action[1]
            self.observationStr = self.actionDig(toolToDig)
    ...
    def calculateScore(self):
        ...
        for obj in allObjects:
            ...
            if type(obj) == Hole:
                ...
                soil_added = False
                watered = False
                for obj_hole in obj.contains:
                    if type(obj_hole) == Tree:
                        tree_planted = True
                    if type(obj_hole) == Soil:
                        soil_added = True
                        if obj_hole.getProperty("wet"):
                            watered = True
