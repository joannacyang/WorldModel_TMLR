...
class ElectricalObject(GameObject):
    def __init__(self, name, conductive=True, resistance=0):
        ...
        self.properties["connects"] = {
            "terminal1": None,
            "terminal2": None,
        }  # the value is (object, terminal), where terminal is the object connected to, and terminal of the terminal of the connected object that is used to connect this object
    ...
    def disconnect(self, terminal):
        if self.connects[terminal] is not None:
            ...
            obj_connects_to.properties["connects"][connected_terminal] = None
        self.connects[terminal] = None
        self.properties["connects"][terminal] = None
...
class Multimeter(ElectricalObject):
    def __init__(self, mode="voltage"):
        ...
        self.properties[
            "mode"
        ] = mode  # multimeter has three modes: voltage, current, resistance
    ...
    def measure(self):
        # voltage mode
        if self.properties["mode"] == "voltage":
            return 0, "V"
        elif self.properties["mode"] == "current":
            return 0, "A"
        elif self.properties["mode"] == "resistance":
            total_resistance = 0
            current_obj = self
            ...
            while True:
                ...
                if current_obj.connects[current_terminal] is None:
                    ...
                    # if a non-conductive object is in the loop, total resistance will be infinite
                        ...
                        if (
                            total_resistance == "inf"
                            or next_obj.getProperty("resistance") == "inf"
                        ):
                            total_resistance = "inf"
                        else:
                            total_resistance += next_obj.getProperty("resistance")
    ...
    def makeDescriptionStr(self, makeDetailed=False):
        ...
        if number == "inf":
            return f"a {self.name}, which is in {self.properties['mode']} mode and reads INF {unit}"
        else:
            return f"a {self.name}, which is in {self.properties['mode']} mode and reads {number}{unit}"
...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        multimeter = Multimeter(
            mode=self.random.choice(["voltage", "current", "resistance"])
        )
        ...
        resistance = self.random.choices(range(1, 51), k=num_resistors)
        ...
        for i in range(num_resistors):
            resistor = ElectricalObject(
                f"resistor {resistor_ids[i]}", resistance=resistance[i]
            )
        ...
        for objReferent1, objs1 in allObjects.items():
            for obj1 in objs1:
                for mode in ["voltage", "current", "resistance"]:
                    self.addAction(
                        f"set {objReferent1} to {mode} mode", ["set", obj1, mode]
                    )
            ...
            for objReferent2, objs2 in allObjects.items():
                for obj1 in objs1:
                    for obj2 in objs2:
                        if (
                            obj1 != obj2
                            and obj1.getProperty("is_electrical_object")
                            and obj2.getProperty("is_electrical_object")
                        ):
    ...
    def actionConnect(self, obj1, terminal_1, obj2, terminal_2):
        # at least one of the two objects should be a wire or a multimeter
        if not (
            obj1.getProperty("is_wire")
            or obj2.getProperty("is_wire")
            or type(obj1) == Multimeter
            or type(obj2) == Multimeter
        ):
        ...
        obj1.properties["connects"][terminal_1] = (obj2.name, terminal_2)
        obj2.connects[terminal_2] = (obj1, terminal_1)
        obj2.properties["connects"][terminal_2] = (obj1.name, terminal_1)
        return (
            f"Successfully connect {obj1.name} {terminal_1} to {obj2.name} {terminal_2}"
        )

    # Answer
    def actionAnswer(self, resistance):
        self.answer_resistance = resistance
        return f"You believe the resistance of the resistor {self.target_resistor_id} is {resistance} ohm."

    # Set the multimeter to a new mode
    def actionSet(self, multimeter, mode):
        # check input multimeter type
        if type(multimeter) != Multimeter:
            return "You cannot set that."
        # check if the mode is valid
        elif mode not in ["voltage", "current", "resistance"]:
            return "Unknown mode."
        else:
            multimeter.properties["mode"] = mode
    ...
    def step_action(self, actionStr):
        ...
        if actionVerb == "look around":
            ...
            multimeter = action[1]
            mode = action[2]
            self.observationStr = self.actionSet(multimeter, mode)
