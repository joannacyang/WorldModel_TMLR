...
class BalanceScale(Container):
    ...
    def placeObjectInContainer(self, obj):
        return (
            f"You can't put {obj.name} on a balance scale directly. Put it on one side of the scale.",
            False,
        )
...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        for color, mass in zip(colors, weights):
            ...
            self.useful_info.append((cube, mass))
    ...
    def calculateScore(self):
        ...
        for obj in allObjects:
            if type(obj) == Cube:
                if (
                    obj.getProperty("weight") == self.max_weight
                    and obj.parentContainer != self.answer_box
                ):
                    completed = False
