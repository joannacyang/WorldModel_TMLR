# volume-stone.py
# based on volume-container.py
# ruoyao wang (apr 28/2023)
...
class Container(GameObject):
    ...
    def takeObjectFromContainer(self, obj):
        ...
        if obj.getProperty("isLiquid"):
            return (
                f"The {obj.name} is liquid and cannot be taken directly.",
                None,
                False,
            )
            ...
            for obj in containedObjects:
                ...
                if obj.getProperty("isWaterContainer"):
                    ...
                    if foundObject is None:
                        water = Water(
                            min(
                                obj.getProperty("volume"),
                                self.properties["water_out_per_tick"],
                            )
                        )
                        obj.addObject(water)
                    # If it contains water, add water
                    else:
                        foundObject.properties["volume"] = min(
                            obj.getProperty("volume"),
                            foundObject.properties["volume"]
                            + self.properties["water_out_per_tick"],
                        )
...
class MeasuringCup(Container):
    def __init__(self, volume):
        ...
        self.properties[
            "liquid"
        ] = False  # whether there is liquid in the measuring cup
        self.properties["volume"] = volume
    ...
    def addObject(self, obj):
        ...
        if obj.getProperty("isLiquid"):
            if self.properties["liquid"]:
                ...
                liquid.properties["volume"] += obj.properties["volume"]
            else:
                self.properties["liquid"] = True

        containedVolume = self.getContainedVolume()

        # Remove extra liquid if the container is full
        if containedVolume > self.properties["volume"]:
            if self.properties["liquid"]:
                liquid = self.getLiquid()
                if (
                    containedVolume - self.properties["volume"]
                    > liquid.properties["volume"]
                ):
                    del liquid
                    self.properties["liquid"] = False
                else:
                    liquid.properties["volume"] -= (
                        containedVolume - self.properties["volume"]
                    )

    def getContainedVolume(self):
        containedVolume = 0
        for obj in self.contains:
            containedVolume += obj.properties["volume"]
        return containedVolume

    # Get liquid contained
    # There is only one liquid water in this game, so we don't consider the situation of multiple types of liquid
    def getLiquid(self):
        for obj in self.contains:
            if obj.getProperty("isLiquid"):
                return obj
    ...
    def makeDescriptionStr(self, makeDetailed=False):
        ...
        if containedVolume > 0:
            contained_items = ", ".join(
                [obj.makeDescriptionStr() for obj in self.contains]
            )
            if self.properties["liquid"]:
                outStr += f" which reads {containedVolume} mL. In it, you see {contained_items}."
            else:
...
class TextGame:
    ...
    def getTaskDescription(self):
        return f"Your task is to figure out the volume of the stone."
            ...
            for objReferent2, objs2 in allObjects.items():
                for obj1 in objs1:
                    for obj2 in objs2:
                        if obj1 != obj2:
                            self.addAction(
                                "pour " + objReferent1 + " into " + objReferent2,
                                ["pour", obj1, obj2],
                            )
    ...
    def actionPour(self, water, target):
        ...
        # Cannot pour to the same container
        elif target == water.parentContainer:
            return f"The {water.name} is already in {target.name}."
        else:
            ...
            if water.getProperty("volume") > target.getProperty("volume"):
                water.properties["volume"] = water.getProperty(
                    "volume"
                ) - target.getProperty("volume")
    ...
    def calculateScore(self):
        ...
        if self.agent_answer_volume is not None:
            if self.answer_volume == self.agent_answer_volume:
                self.score += 1
                self.gameOver = True
                self.gameWon = True
            else:
