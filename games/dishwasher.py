...
class DishWasher(Container, Device):
    def __init__(self):
        ...
        self.properties[
            "isMoveable"
        ] = False  # A dishwasher is too heavy to move (and doesn't really need to be moved for this simulation)

        # Set critical properties
        self.properties[
            "cycleStage"
        ] = 0  # The current stage of the dishwasher's cycle.  0 = not running, 1 = washing, 2 = finished
        self.properties[
            "finishedCycle"
        ] = False  # Set to True when the dishwasher has finished washing the dishes.  Reset when it's open.
...
class Dish(Container):
    ...
    def makeDescriptionStr(self, makeDetailed=False):
        ...
        effectiveContents = []
        if self.properties["isDirty"]:
            effectiveContents.append(
                "half-eaten pieces of " + self.properties["foodMessName"]
            )
...
class DishSoapBottle(Device):
    ...
    def useWithObject(self, patientObject):
        ...
        if type(patientObject) == Dish:
            ...
            return (
                "You squirt some dish soap on the "
                + patientObject.getReferents()[0]
                + ".",
                True,
            )
    ...
    def initializeWorld(self):
        ...
        for i in range(numDirtyDishes):
            ...
            dish = Dish(
                ...
                isDirty=True,
                foodName=foodName,
                containerPrefix=containerPrefixes[dishType],
            )
        ...
        for objReferent1, objs1 in allObjects.items():
            for objReferent2, objs2 in allObjects.items():
                for obj1 in objs1:
                    for obj2 in objs2:
                        self.addAction(
                            f"eat {objReferent1} with {objReferent2}",
                            ["eat", obj1, obj2],
                        )
