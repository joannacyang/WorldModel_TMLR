...
# Task: Create a micro-simulation that models how to measure and compute the volume of a box using a ruler.
# Environment: emergency room
# Task-critical Objects: Box, Ruler
# High-level object classes: None
# Critical properties: size (Box)
# Actions: look, inventory, examine, take/put object, measure, answer
# Distractor Items: Box
# Distractor Actions: None
# High-level solution procedure: measure the length, width, height of the target box, compute and answer the volume of the box
...
class Box(GameObject):
    def __init__(self, name, l, w, h):
        GameObject.__init__(self, name)
        # Set critical properties
        self.properties["size"] = {"length": l, "width": w, "height": h}
...
class Ruler(GameObject):
    ...
    # When using the ruler on an object, the length of the object can be measured by the ruler.
    def useWithObject(self, obj, edge):
        ...
        elif edge not in obj.properties["size"]:
            return "I do not understand that."
        else:
            return (
                f"The {edge} of the {obj.name} is {obj.properties['size'][edge]} cm",
                True,
            )
...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        # generate 2-8 boxes with random size and color
        # one box is the target box and the others are distractors
        num_boxes = self.random.randint(2, 8)
        sizes = self.random.choices(range(1, 6), k=num_boxes * 3)
        for i in range(num_boxes):
            box = Box(
                f"{possible_colors[i]} box",
                sizes[3 * i] * 10,
                sizes[3 * i + 1] * 10,
                sizes[3 * i + 2] * 10,
            )
            # use the first box as the target box
            if i == 0:
                self.target_box_color = possible_colors[0]
                self.target_box_volume = (
                    box.properties["size"]["length"]
                    * box.properties["size"]["width"]
                    * box.properties["size"]["height"]
                )

                self.useful_info.append(box)
            world.addObject(box)
    ...
    def getTaskDescription(self):
        return (
            f"Your task is to figure out the volume of the {self.target_box_color} box."
        )
                    ...
                    for obj2 in objs2:
                        if obj1 != obj2:
                            for edge in ["length", "width", "height"]:
                                self.addAction(
                                    f"measure the {edge} of the {objReferent1} with the {objReferent2}",
    ...
    def actionMeasure(self, patientObject, deviceObj, edge):
        ...
        if type(deviceObj) == Ruler:
            ...
            if type(deviceObj.parentContainer) == Agent:
