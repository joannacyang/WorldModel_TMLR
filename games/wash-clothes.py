...
class WashingMachine(Container, Device):
    def __init__(self):
        ...
        self.properties[
            "isMoveable"
        ] = False  # A washing machine is too heavy to move (and doesn't really need to be moved for this simulation)
        # Set critical properties
        self.properties[
            "cycleStage"
        ] = 0  # The current stage of the washing machine's cycle.  0 = not running, 1 = washing, 2 = finished
        self.properties[
            "finishedCycle"
        ] = False  # Set to True when the washing machine has finished washing the dishes.  Reset when it's open.
...
class Dryer(Container, Device):
    def __init__(self):
        ...
        self.properties[
            "isMoveable"
        ] = False  # A dryer is too heavy to move (and doesn't really need to be moved for this simulation)
        # Set critical properties
        self.properties[
            "cycleStage"
        ] = 0  # The current stage of the dryer's cycle.  0 = not running, 1 = drying, 2 = finished
        self.properties[
            "finishedCycle"
        ] = False  # Set to True when the dryer has finished drying the clothes.  Reset when it's open.
...
class Clothes(GameObject):
    ...
    def getReferents(self):
        ...
        return [
            self.name,
            f"{is_dirty}{self.name}",
            f"{is_wet}{self.name}",
            f"{is_dirty}{is_wet}{self.name}",
        ]
...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        for i in range(self.num_dirty_clothes):
            ...
            self.dirty_clothes.append(clothes)
        ...
        for i in range(self.num_dirty_clothes, len(clothes_names)):
            ...
            self.clean_clothes.append(clothes)
            # Add the clothes to the environment
            world.addObject(clothes)
    ...
    def calculateScore(self):
        ...
        for obj in allObjects:
            if type(obj) == Clothes:
                if (
                    obj.getProperty("isDirty") == True
                    or obj.getProperty("isWet") == True
                    or obj.parentContainer != self.answer_box
                ):
                    all_complete = False
                    break
