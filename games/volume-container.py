...
class TextGame:
    ...
    def initializeWorld(self):
        ...
        water_containers_volume = self.random.choices(
            range(50, 200), k=num_water_containers
        )
        target_id = self.random.randint(0, num_water_containers - 1)

        for i in range(num_water_containers):
            water_container = WaterContainer(
                water_containers[i], water_containers_volume[i]
            )
