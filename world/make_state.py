...
def recover_game_state_from_partial(curr_state, partial_change, has_score=False):
    ...
    # print("=" * 100)
    # print(curr_state)
    # print(partial_change)
    # print("=" * 100)
    # print(
    #     [state for state in partial_change["modified"]]
    # )
...
def get_state(text_game, last_action, max_UUID, game_name):
    ...
    global_state = {
        "observation": text_game.observationStr,
        "look": text_game.actionLook(),
        "inventory": text_game.actionInventory()
        if hasattr(text_game.__class__, "actionInventory")
        else "No inventory available.",
        ...
        for key in target:
            if key not in pred:
                ...
                if pred[key] != target[key]:
                    num_errors += 1
                    out_str.append(
                        f"Wrong value for the key {key}. Prediction: {pred[key]}, Target: {target[key]}"
                    )
        if num_errors == 0:
...
def evaluate(prediction, target, last_action, evaluate_score=False):
        ...
        for uuid in target:
            if uuid not in prediction:
                ...
                if "properties" not in prediction[uuid]:
                    out_str += (
                        f'No prediction of properties: {target[uuid]["name"]}\n\n'
                    )
                    num_errors += 1
                else:
                    ...
                    for key in t:
                        if key not in p:
                            out_str += (
                                f'Missing key: {key} for {target[uuid]["name"]}\n\n'
                            )
                            num_errors += 1
                        else:
                            if type(t[key]) in [list, tuple] and type(p[key]) in [
                                list,
                                tuple,
                            ]:
                                # We don't care it is a list/tuple
                ...
                if "contains" not in prediction[uuid]:
                    ...
                    if sorted(target[uuid]["contains"]) != sorted(
                        prediction[uuid]["contains"]
                    ):
                        out_str += (
                            f"Difference in contains of {target[uuid]['name']}:\n"
                        )
    ...
    if evaluate_score:
        out_str += (
            f"Total errors: {num_errors}, Total score errors: {num_errors_score}\n"
        )
    else:
    ...
    for uuid in state_1:
        ...
        for key in property_1:
            if key not in property_2:
                ...
                if type(property_1[key]) in [list, tuple] and type(property_2[key]) in [
                    list,
                    tuple,
                ]:
                    # We don't care it is a list/tuple
        ...
        # We don't care the order of contains
        if uuid not in diffs and sorted(state_1[uuid]["contains"]) != sorted(
            state_2[uuid]["contains"]
        ):
            diffs[uuid] = (state_1[uuid], state_2[uuid])
...
def get_state_diff_detail_v2(state_1, state_2):
    ...
    for uuid in state_1:
        ...
        # We don't care the order of contains
        # if sorted(state_1[uuid]["contains"]) != sorted(state_2[uuid]["contains"]):
        if sorted(state_1[uuid].get("contains", [])) != sorted(
            state_2[uuid].get("contains", [])
        ):
            diffs["modified"].append(("contains", state_1[uuid], state_2[uuid], 0))
            is_same = False
        else:
